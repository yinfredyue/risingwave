...
pub trait Page<K: Ord + Clone, V: Clone>: Clone {
    type Iter: PageIterator<K, V>;

    fn new_page(max_page_size: usize) -> Self;
    fn new_iterator(self: &Arc<Self>) -> Self::Iter;
    fn seek(&self, key: &K) -> Option<&(K, V)>;
    fn smallest(&self) -> Option<&K>;
    fn largest(&self) -> Option<&K>;
    fn split(&self) -> Vec<Arc<Self>>;
    fn merge(&self, other: &Self) -> Arc<Self>;
    fn size(&self) -> usize;
    fn record_number(&self) -> usize;
    fn insert(&mut self, records: Vec<(K, V)>);
...
pub trait PageIterator<K: Ord + Clone, V: Clone>: Clone {
...
pub struct LeafNodeIterator<K: Ord + Clone, V: Clone> {
    page: Arc<LeafPage<K, V>>,
    cursor: usize,
}

impl<K: Ord + Clone, V: Clone> PageIterator<K, V> for LeafNodeIterator<K, V> {
    fn seek(&mut self, key: &K) {
        ...
        self.cursor = match self
            .page
            .data
            .binary_search_by(|node| node.0.cmp(key))
        {
            Ok(idx) => idx,
            Err(upper) => upper,
        };
    ...
    fn value(&self) -> &V {
        debug_assert!(self.cursor < self.page.data.len());
        &self.page.data[self.cursor].1
    }

    fn key(&self) -> &K {
        debug_assert!(self.cursor < self.page.data.len());
        &self.page.data[self.cursor].0
    }
}

#[derive(Clone, Default)]
pub struct LeafPage<K: Ord + Clone, V: Clone> {
    data: Vec<(K, V)>,
    smallest: Option<K>,
    largest: Option<K>,
    max_page_size: usize,
...
impl<K: Ord + Clone, V: Clone> Page<K, V> for LeafPage<K, V> {
    type Iter = LeafNodeIterator<K, V>;
    fn new_page(max_page_size: usize) -> Self {
        LeafPage::<K, V> {
            data: vec![],
            largest: None,
    ...
    fn seek(&self, key: &K) -> Option<&(K, V)> {
        ...
        let idx = match self.data.binary_search_by(|node| node.0.cmp(key)) {
            Ok(idx) => idx,
            Err(upper) => upper,
        };
    ...
    fn split(&self) -> Vec<Arc<LeafPage<K, V>>> {
        ...
        while start_idx < self.data.len() {
            ...
            let key = if start_idx == 0 {
                ...
                Some(self.data[start_idx].0.clone())
            };
            nodes.push(Arc::new(Self {
                data: new_data,
                smallest: key,
                largest: self.data[end_idx - 1].0.clone(),
    ...
    fn merge(&self, other: &LeafPage<K, V>) -> Arc<LeafPage<K, V>> {
    ...
    fn insert(&mut self, mut tables: Vec<T>) {
        ...
        self.largest = self.data.last().unwrap().0.clone();
    }
...
#[derive(Clone)]
pub struct BTreePage<K: Ord + Clone, V: Clone, P: Page<K, V>> {
    son: Vec<Arc<P>>,
    smallest: Vec<u8>,
    largest: Vec<u8>,
    record_number: usize,
...
#[derive(Clone)]
pub struct BTreePageIterator<K: Ord + Clone, V: Clone, P: Page<K, V>> {
    page: Arc<BTreePage<K, V, P>>,
    cursor: usize,
    iter: Option<P::Iter>,
}

impl<K, V, P> PageIterator<K, V> for BTreePageIterator<K, V, P>
where
    K: Ord + Clone,
    V: Clone,
    P: Page<K, V>,
...
impl<K, V, P> Page<K, V> for BTreePage<K, V, P>
where
    K: Ord + Clone,
    V: Clone,
    P: Page<K, V>,
{
    type Iter = BTreePageIterator<K, V, P>;
    ...
    fn insert(&mut self, records: Vec<R>) {
        ...
        let key = records.first().unwrap().key();
        ...
        for r in records {
            if idx + 1 < self.son.len()
                && r.key().ge(self.son[idx + 1].smallest())
                && !cur_records.is_empty()
            {
                ...
                while idx + 1 < self.son.len() && r.key().ge(self.son[idx + 1].smallest()) {
                    idx += 1;
                }
...
impl<T: ComparableRecord, P: Page<T>> BTree<T, P> {
    ...
    pub fn add(&self, mut to_add: Vec<T>) -> Self {
        ...
        to_add.sort_by(|a, b| a.key().cmp(b.key()));
        node.insert(to_add);
        Self {
            node: Arc::new(node),
            _phantom: PhantomData,
    ...
    pub fn replace(&self, mut to_del: Vec<T>, mut to_add: Vec<T>) -> Self {
        let mut node = self.node.as_ref().clone();
        if !to_del.is_empty() {
            to_del.sort_by(|a, b| a.key().cmp(b.key()));
            node.delete(to_del);
        }
        if !to_add.is_empty() {
            to_add.sort_by(|a, b| a.key().cmp(b.key()));
