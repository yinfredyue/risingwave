...
impl StreamFragmenter {
    // TODO V2, see if this is requried .
    ...
    fn generate_fragment_graph(
        ...
        //let plan_node = self.rewrite_stream_node(state, plan_node)?;
        self.build_and_add_fragment(state, plan_node)?;
        Ok(())
    }

    /// Use the given `plan_node` to create a fragment and add it to graph.
    fn build_and_add_fragment(
        ...
        let node = self.build_fragment(state, &mut fragment, plan_node)?;
    ...
    fn build_fragment(
        ...
        mut plan_node: PlanRef,
    ) -> Result<PlanRef> {
        // Update current fragment based on the node we're visiting.
        match plan_node.to_stream_prost_body()? {
        ...
        Self::assign_local_table_id_to_stream_node(state, &mut plan_node);

        // handle join logic
        match plan_node.to_stream_prost_body().as_mut().unwrap() {
            ...
            NodeBody::HashJoin(hash_join_node) => {
                if hash_join_node.is_delta_join {
                        ...
                        && hash_join_node.condition.is_none()
                    {
                        ...
                        // TODO V2 Implement Delta Join
                        panic!(
                            "delta join is not yet supported"
                        );
                    } else {
                        panic!(
        ...
        let inputs = std::mem::take(&mut plan_node.inputs());
        // Visit plan children.
        let inputs = inputs
            .into_iter()
            .map(|mut child_node| -> Result<PlanNode> {
                match child_node.get_node_body()? {
                    NodeBody::Exchange(_) if child_node.inputs().is_empty() => {
                    ...
                    NodeBody::Exchange(exchange_node) => {
                        ...
                        let node_body = exchange_node.to_stream_prost_body();
                        assert_eq!(child_node.input.len(), 1);
                        let child_fragment =
                            self.build_and_add_fragment(state, child_node.inputs().remove(0))?;
                        state.fragment_graph.add_edge(
                            child_fragment.fragment_id,
                            current_fragment.fragment_id,
                            StreamFragmentEdge {
                                dispatch_strategy: node_body.strategy.clone(),
                                same_worker_node: false,
                                link_id: child_node.id().0,
                            },
                        );

                        let is_simple_dispatcher =
                            node_body.strategy.get_type()? == DispatcherType::Simple;
    ...
    pub fn build_graph(plan_node: PlanRef) -> StreamFragmentGraphProto {
        ...
        let BuildFragmentGraphState {
            ...
            fragmenter
                .generate_fragment_graph(&mut state, plan_node)
                .unwrap();
            state
        };
