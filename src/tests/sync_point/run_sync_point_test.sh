...
# cargo test -p risingwave_sync_point_test -- --nocapture --list --format=terse| awk '/: test/{print substr($1, 1, length($1)-1)}' |xargs -I{} cargo test -p risingwave_sync_point_test {}
