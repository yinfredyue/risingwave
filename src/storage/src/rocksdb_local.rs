...
impl StateStore for RocksDBStateStore {
    ...
    fn get<'a>(&'a self, key: &'a [u8], _epoch: u64) -> Self::GetFuture<'_> {
        async move {
            let v = self.storage().await.get(key).unwrap();
            Ok(v)
        }
    }
    ...
    fn ingest_batch(
        ...
        async move {
            Ok(self.storage().await.write_batch(kv_pairs))
        }
    }
            ...
            B: AsRef<[u8]> + Send,
    {
        async move {
            ...
            Ok(RocksDBStateStoreIter::new(self.clone(), range).await.unwrap())
        }
    }
...
impl RocksDBStorage {
    ...
    fn write_batch(&self, kv_pairs: Vec<(Bytes, StorageValue)>) -> u64 {
        ...
        for (key, value) in kv_pairs {
            ...
            if let Some(value) = value {
                wb.put(key.as_ref(), value.as_ref()).unwrap();
            } else {
                wb.delete(key.as_ref()).unwrap();
            }
        }
    ...
    fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        Ok(db.get(&seek_key).unwrap().map(|v| Bytes::from(v.to_vec())))
    }

    async fn iter(&self) -> DBIterator<Arc<DB>> {
        let db = self.db.clone();
        DBIterator::new(db, ReadOptions::default())
    }
}
